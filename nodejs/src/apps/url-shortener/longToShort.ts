...
export function longToShort(longUrl: string, indexRange: [number, number] /* 0-238326*/): {
  generatedId: number
  shortened: string
} {
  ...
  if (indexRange[0] === indexRange[1]) {
    ...
    index = indexRange[0] + Math.floor(buffer.slice(4, 7).reduce((p, c, i) => p + c * Math.pow(256, i)) / divider)
  }
